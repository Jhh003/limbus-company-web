...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/api/admin/migrate-v2') {
      return handleMigrateV2(request, env, headers);
    }
...
async function handleGuides(request, env, headers, path) {
  ...
  if (path === '/api/guides/my-submissions') {
      return handleMySubmissions(request, env, headers);
    }

    // 用户内容管理
    if (path === '/api/user/contributions') {
      return handleUserContributions(request, env, headers);
    }
    
    if (path.match(/^\/api\/user\/content\/(guide|ranking)\/\d+$/)) {
      return handleUserContentDelete(request, env, headers, path);
    }
